...
- [`actions/`](actions/README.md) — **one "Learn more" page per operation** (generated by
  `tools/gen_action_docs.py`): every parameter explained, plus copy-paste `input_data`
  variations. Each imported action links its page from the About tab (`externalDocs` + the
  `Learn more:` suffix on the description).
